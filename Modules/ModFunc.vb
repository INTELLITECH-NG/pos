...
Module ModFunc
    Dim st As Object
    Public Function ReadLC() As Object
        Using sr As StreamReader = New StreamReader(Application.StartupPath & "\LC.txt")
            st = sr.ReadLine()
        End Using
        Return st
    End Function
    ...
    Sub LedgerSave(ByVal a As DateTime, ByVal b As String, ByVal c As String, ByVal d As String, ByVal e As Decimal, ByVal f As Decimal, ByVal g As String, ByVal h As String)
        con = New SqlConnection(cs)
        con.Open()
        Dim cb As String = "insert into LedgerBook(Date, Name, LedgerNo, Label,Debit,Credit,PartyID,AccLedger) Values (@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8)"
        cmd = New SqlCommand(cb)
