...
Public Class frmPurchaseOrder
    ...
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb1 As String = "insert into PurchaseOrder_Join(PurchaseOrderID,ProductID,Qty,PriceBrownnit,Amount) VALUES (" & txtPO_ID.Text & ",@d1,@d2,@d3,@d4)"
    ...
    Private Sub cmbProductName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbProductName.SelectedIndexChanged
        Try
            ...
            cmd.CommandText = "SELECT PID,RTRIM(PurchaseUnit),RTRIM(PurchaseCost),RTRIM(Barcode) from Product where ProductName=@d1"
            ...
            If rdr.Read() Then
                ...
                txtBarcode.Text = rdr.GetValue(3).ToString()
            End If
            If (rdr IsNot Nothing) Then
                rdr.Close()
            End If
    ...
    Sub GetQty_W()
        Try
            ...
            rdr = cmd.ExecuteReader()
            If rdr.Read() Then
                lblQty_W.Visible = True
                lblQty_W.Text = rdr.GetValue(0)
            Else
                lblQty_W.Visible = True
                lblQty_W.Text = "0.00"
            End If
    ...
    Sub GetQty_S()
        Try
            ...
            con.Open()
            cmd = con.CreateCommand()
            cmd.CommandText = "Select IsNull(Sum(Qty),0) from temp_stock_Company,Product where Temp_Stock_Company.ProductID=Product.PID and ProductName=@d1 group by ProductName"
            cmd.Parameters.AddWithValue("@d1", cmbProductName.Text)
            rdr = cmd.ExecuteReader()
            If rdr.Read() Then
    ...
    Private Sub btnAdd_Click(sender As System.Object, e As System.EventArgs) Handles btnAdd.Click
        Try
            ...
            If DataGridView1.Rows.Count = 0 Then
                DataGridView1.Rows.Add(Val(txtProductID.Text), cmbProductName.Text, txtBarcode.Text, Val(txtQty.Text), Val(txtPricePerQty.Text), Val(txtTotalAmount.Text))
                Dim k As Double = 0
                k = SubTotal()
                k = Math.Round(k, 2)
                txtSubTotal.Text = k
    ...
    Private Sub txtBarcode_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles txtBarcode.KeyDown
        Try
            If e.KeyCode = Keys.Enter Then
                ...
                con.Open()
                cmd = con.CreateCommand()
                cmd.CommandText = "SELECT PID,RTRIM(PurchaseUnit),RTRIM(PurchaseCost),RTRIM(ProductName) from Product where Barcode=@d1"
                ...
                If rdr.Read() Then
                    ...
                    lblUnit.Text = rdr.GetValue(1)
                    txtPricePerQty.Text = rdr.GetValue(2)
                    cmbProductName.Text = rdr.GetValue(3).ToString()
                End If
                If (rdr IsNot Nothing) Then
                    rdr.Close()
                End If
