...
Public Class frmStockAdjustment_Warehouse
        ...
        Try
            ...
            If RowsAffected > 0 Then
                If rbPlus.Checked = True Then
                    con = New SqlConnection(cs)
                    con.Open()
                    Dim cb1 As String = "Update Temp_Stock set qty=qty - " & Val(txtQty.Text) & " where ProductID=@d1 and Barcode=@d2 and Warehouse=@d3"
    ...
    Private Sub btnSave_Click(sender As System.Object, e As System.EventArgs) Handles btnSave.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "insert into StockAdjustment_Warehouse(SA_ID, ProductID, Barcode, Date, AdjustmentType, Qty, Reason,Warehouse) VALUES (@d1,@d2,@d3,@d4,@d5,@d6,@d7,@d8)"
    ...
    Private Sub cmbProductName_SelectedIndexChanged(sender As System.Object, e As System.EventArgs) Handles cmbProductName.SelectedIndexChanged
        Try
            ...
            cmd.CommandText = "SELECT PID,RTRIM(ProductCode),RTRIM(Temp_Stock.Barcode) from Product,Temp_Stock where Product.PID=Temp_Stock.ProductID and ProductName=@d1 and Warehouse=@d2"
    ...
    Sub GetQty_S()
        Try
            ...
            con.Open()
            cmd = con.CreateCommand()
            cmd.CommandText = "Select IsNull(Sum(Qty),0) from Temp_Stock,Product where Temp_Stock.ProductID=Product.PID and ProductID=@d1 and Temp_Stock.Barcode=@d2 and Warehouse=@d3 group by ProductID,Temp_Stock.Barcode"
            cmd.Parameters.AddWithValue("@d1", Val(txtProductID.Text))
            cmd.Parameters.AddWithValue("@d2", txtBarcode.Text)
            cmd.Parameters.AddWithValue("@d3", cmbWarehouse.Text)
            rdr = cmd.ExecuteReader()
    ...
    Private Sub txtBarcode_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles txtBarcode.KeyDown
        Try
            ...
            If e.KeyCode = Keys.Enter Then
                ...
                con.Open()
                cmd = con.CreateCommand()
                cmd.CommandText = "SELECT PID,RTRIM(ProductCode),RTRIM(ProductName) from Product,Temp_Stock where Product.PID=Temp_Stock.ProductID and Temp_Stock.Barcode=@d1 and Warehouse=@d2"
    ...
    Private Sub btnUpdate_Click(sender As System.Object, e As System.EventArgs) Handles btnUpdate.Click
        ...
        Try
            ...
            con = New SqlConnection(cs)
            con.Open()
            Dim cb As String = "Update StockAdjustment_Warehouse set ProductID=@d2, Barcode=@d3, Date=@d4, AdjustmentType=@d5, Qty=@d6, Reason=@d7,Warehouse=@d8 where SA_ID=@d1"
            cmd = New SqlCommand(cb)
            cmd.Parameters.AddWithValue("@d1", Val(txtAdjustmentID.Text))
            cmd.Parameters.AddWithValue("@d2", Val(txtProductID.Text))
            cmd.Parameters.AddWithValue("@d3", txtBarcode.Text)
            ...
            If rbPlus.Checked = True Then
                con = New SqlConnection(cs)
                con.Open()
                Dim cb1 As String = "Update Temp_Stock set qty=qty + ( " & Val(txtQty.Text) & " - " & Val(txtQ.Text) & ") where ProductID=@d1 and Barcode=@d2 and Warehouse=@d3"
                cmd = New SqlCommand(cb1)
                cmd.Parameters.AddWithValue("@d1", Val(txtProductID.Text))
                cmd.Parameters.AddWithValue("@d2", txtBarcode.Text)
                cmd.Parameters.AddWithValue("@d3", cmbWarehouse.Text)
