        ...
        Try
            If dgw.Rows.Count > 0 Then
                If lblSet.Text = "PO" Then
                    ...
                    con = New SqlConnection(cs)
                    con.Open()
                    Dim sql As String = "SELECT PurchaseOrder_Join.ProductID, RTRIM(Product.ProductName),RTRIM(Product.Barcode), PurchaseOrder_Join.Qty, PurchaseOrder_Join.PriceBrownnit, PurchaseOrder_Join.Amount FROM PurchaseOrder INNER JOIN PurchaseOrder_Join ON PurchaseOrder.PO_ID = PurchaseOrder_Join.PurchaseOrderID INNER JOIN Product ON PurchaseOrder_Join.ProductID = Product.PID  where PurchaseOrder_Join.PurchaseOrderID=" & dr.Cells(0).Value & ""
                    cmd = New SqlCommand(sql, con)
                    rdr = cmd.ExecuteReader(CommandBehavior.CloseConnection)
                    frmPurchaseOrder.DataGridView1.Rows.Clear()
                    While (rdr.Read() = True)
                        frmPurchaseOrder.DataGridView1.Rows.Add(rdr(0), rdr(1), rdr(2), rdr(3), rdr(4), rdr(5))
                    End While
                    con.Close()
